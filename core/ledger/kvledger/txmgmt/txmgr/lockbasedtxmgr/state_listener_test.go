...
func TestStateListener(t *testing.T) {
	...
	dummyBlock := cached.WrapBlock(common.NewBlock(1, []byte("dummyHash")))
	...
	txmgr.current = &current{block: cached.WrapBlock(common.NewBlock(2, []byte("anotherDummyHash"))), batch: sampleBatch}
	txmgr.invokeNamespaceListeners()
	assert.Equal(t, 1, ml1.HandleStateUpdatesCallCount())
	assert.Equal(t, 1, ml2.HandleStateUpdatesCallCount())
